...
static char	symbol_type_no_section32(Elf32_Sym *elf_symbol)
{
	unsigned char	st_bind = ELF32_ST_BIND(elf_symbol->st_info);
	unsigned char	st_type = ELF32_ST_TYPE(elf_symbol->st_info);
	uint32_t		st_shndx = elf_symbol->st_shndx;
...
static char	symbol_type32(Elf32_Sym *elf_symbol, Elf32_Shdr *sections)
{
	...
	if ((type = symbol_type_no_section32(elf_symbol)) != '?')
		return (type);
