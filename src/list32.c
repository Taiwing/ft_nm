...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/04/01 19:26:24 by yforeau           #+#    #+#             */
/*   Updated: 2023/04/01 20:16:27 by yforeau          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				list32(t_list **dest, t_nm_file *file)
{
	...
	if (!(sh_symtab = get_section_header32(sections, count, SHT_SYMTAB)))
		return (1);
	else if (!(sh_strtab = get_section_header32(sections, count, SHT_STRTAB)))
		return (1);
	else if (!(shstrtab = get_section32(file, sh_shstrtab)))
		return (1);
